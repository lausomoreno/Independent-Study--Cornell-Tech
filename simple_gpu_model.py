...
class GPUSystemParameters:
    ...
    def __init__(self):
        ...
        # --- Job Arrival Process ---
        self.lambda_arrival = 20.0  # Average jobs per time slot (Poisson rate)
...
class PopulationDynamics:
    ...
    def evolve(self, state, control, arrivals):
        """
        Evolve population one time step.
        ...
        # --- Process arrivals ---
        # New arrivals get assigned to GPUs (assume infinite GPUs for now)
        for (tau, s), count in arrivals.items():
            # These jobs get GPUs and enter the busy population
            new_state.set_count(tau, s, 
                              new_state.get_count(tau, s) + count)
            new_state.n_idle -= count
        
        # Copy idle count (will be updated as jobs finish)
        ...
        for (tau, s) in state.get_all_states():
            ...
            for k in range(self.params.K):
                ...
                # Number of GPUs running at frequency k
                ...
                if tau_next <= 0:
                    # Job done! GPU goes back to idle
                    new_state.n_idle += n_at_k
                else:
                    # Job continues in new state
                    # Round to nearest 0.25 to keep state space discrete
...
class GPUSimulator:
    ...
    def run(self):
        ...
        for t in range(self.params.T_sim):
            ...
            # 3. Evolve state
            self.state = self.dynamics.evolve(self.state, control_t, arrivals_t)
    ...
    def plot_results(self):
        ...
        plt.tight_layout()
        plt.savefig(r'C:/Users/lauso/OneDrive/Desktop/Cornell/AI flexible demand response/New model images', dpi=150, bbox_inches='tight')
